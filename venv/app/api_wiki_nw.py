...
def config_request_demande(chaine):
    """Paramétre de l'API."""
    parametres = {
       "action": "query",
       "format": "json",
...
def api_wikipedia(
    ...
    if type(terme_recherche) is dict:
        chaine = terme_recherche[lat] + "|" + terme_recherche[lng]
        parametres = config_request(chaine)
        r = session.get(url=adresse_api, params=parametres)
    else:
        parametres = config_request_demande(terme_recherche)
        r = session.get(url=adresse_api, params=parametres)
    reponse = r.json()
    return reponse
