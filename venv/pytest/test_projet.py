...
class Testmain:
    ...
    def test_json(self):
        sortie_json = self.fichierjson.creation_json(
            "Test demande",
            [[],
             "Test",
             "Un jeune bien élevé comme on les apprécie tant ! ",
             False])
    ...
    def test_geometry_google(self):
        json_test = json.loads(
            '{"html_attributions": [], "results": [{"formatted_address": "test, test", "geometry": {"location": {"lat": "test", "lng": "test"}}}], "status": "OK"}')
    ...
    def test_wiki_informations_snippet(self):
        ...
        assert reponse == info[0]['snippet'] + " Suit ce <a href='https://fr.wikipedia.org/?curid=" + str(
            info[0]['pageid']) + "' >lien</a> et plus d'informaiton tu trouvera !</br>"

    def test_wiki_informations_pages(self):
        section = 'pages'
        info = {'test': {'pageid': 11111, 'extract': "Test"}}
        reponse = script_wiki.informations(info, section)
        assert reponse == info['test']['extract'] + " Suit ce <a href='https://fr.wikipedia.org/?curid=" + str(
            info['test']['pageid']) + "' >lien</a> et plus d'informaiton tu trouvera !</br>"
    ...
    def test_gestion_chaine(self):
        mot = "test"
        phrase = "homonymes ///////***/*/**--*/*/." + mot + \
            " : <div>" + mot + "</div> / <span>" + mot + "</span>"
